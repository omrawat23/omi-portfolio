...
export async function POST(request: NextRequest) {
  try {
    ...
    const historyMessages: OpenAIMessage[] = []; // or filtered/validated history
    ...
    const requestBody = {
      model: 'sonar', // Changed from 'sonar-pro' to 'sonar' to avoid invalid model error
      messages,
      max_tokens: 512,
      temperature: 0.7,
      top_p: 0.8,
    ...
    const response = await fetch(pplxUrl, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Perplexity error body:', errText);
